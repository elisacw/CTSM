...
#include "shr_assert.h"
  ...
  type, public :: cnveg_carbonflux_type
     ...
     real(r8), pointer :: miner_som_resp_patch                      (:)     ! miner respiration during decomposing SOM (gC/m2/s)
     ...
     real(r8), pointer :: npp_growth_patch                          (:)     ! Total C u for growth in FUN / MIMICSplus (gC/m2/s)   
...
contains
  ...
  subroutine InitAllocate(this, bounds, carbon_type, alloc_full_veg)
    ...
    allocate(this%miner_som_resp_patch                      (begp:endp)) ; this%miner_som_resp_patch                      (:) = nan
  ...
  subroutine InitHistory(this, bounds, carbon_type)
    ...
    if (carbon_type == 'c12') then
       ...
       this%symbiont_gr_patch(begp:endp) = spval
       ...
       this%miner_som_resp_patch(begp:endp) = spval
       call hist_addfld1d (fname='MINING_SOM_RESP', units='gC/m^2/s', &
            avgflag='A', long_name='Mining respiration during SOM decomosition', &
            ptr_patch=this%miner_som_resp_patch, default='inactive')
...
! FUN Ends 
  ...
  subroutine SetValues ( this, nvegcpool, &
    ...
    do fi = 1,num_patch
       ...
       this%miner_som_resp_patch(i)                       = value_patch
  ...
  subroutine Summary_carbonflux(this, &
    ...
    do fp = 1,num_soilp
      ...
      ! I don't add C_allocation_to_N_acq here, because in FUN soilc_change_patch dosen't actually go to mycorrhiza pools, it is just respiered?
        if (decomp_method == mimicsplus_decomp) then
          this%ar_patch(p) = this%ar_patch(p) + this%symbiont_gr_patch(p) + this%symbiont_maint_patch(p) + this%miner_som_resp_patch(p)
       end if
